# 26/9/19 messing with pandas and the other planet archive
...
for i in range(len(exoplanet_array)):
    if exoplanet_array[i].detection_type == "Primary Transit": t += 1
    if exoplanet_array[i].detection_type == "Radial Velocity": rv += 1
    if exoplanet_array[i].detection_type == "Microlensing": ml += 1
    if exoplanet_array[i].detection_type == "Imaging": im += 1
print(t, rv, ml, im)

# experimenting with sorting by property
#exoplanet_array.sort(key=lambda x: x.radius)
#for i in range(len(exoplanet_array)):
    #x = exoplanet_array[i].radius
    #if x != "": print(x)